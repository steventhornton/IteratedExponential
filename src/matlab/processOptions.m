...
%                                                                         %
% TO DO                                                                   %
%   - Add type checking for margin, cmap and backgroundColor              %
%                                                                         %
% LICENSE                                                                 %
...
function opts = processOptions(options)
    ...
    % nPre -----------------------------
    if isfield(options, optNames.nPre)
    
        opts.nPre = options.nPre;
    
        % Check that nPre is a positive integer
        if ~isposint(opts.nPre)
            error('nPre option must be a positive integer');
        end
    
    end
    
    % maxLength ------------------------
    if isfield(options, optNames.maxLength)
    
        opts.maxLength = options.maxLength;
    
        % Check that maxLength is a positive integer
        if ~isposint(opts.maxLength)
            error('maxLength option must be a positive integer');
        end
